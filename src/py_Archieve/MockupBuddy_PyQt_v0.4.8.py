# MockupBuddy - PyQt Edition v0.4.8
# ✅ Sliders fixed: Independent value tracking and preview updates
# ✅ Based on 0.4.7 feature-complete version
...
class MockupBuddy(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MockupBuddy v0.4.8")
    ...
    def setup_ui(self):
        ...
        # Size slider
        self.size_slider = self._create_labeled_slider("Size %", 1, 100, self.size, self.size_changed)
        controls.addLayout(self.size_slider)

        # Opacity slider
        self.opacity_slider = self._create_labeled_slider("Opacity %", 0, 100, self.opacity, self.opacity_changed)
        controls.addLayout(self.opacity_slider)

        # Offset X
        self.offset_x_slider = self._create_labeled_slider("Offset X", -200, 200, self.offset_x, self.offset_x_changed)
        controls.addLayout(self.offset_x_slider)

        # Offset Y
        self.offset_y_slider = self._create_labeled_slider("Offset Y", -200, 200, self.offset_y, self.offset_y_changed)
        controls.addLayout(self.offset_y_slider)
    ...
    def _create_labeled_slider(self, label_text, min_val, max_val, default_val, callback):
        layout = QVBoxLayout()
    ...
    def size_changed(self, val): self.size = val; self.update_preview()
    def opacity_changed(self, val): self.opacity = val; self.update_preview()
    def offset_x_changed(self, val): self.offset_x = val; self.update_preview()
    def offset_y_changed(self, val): self.offset_y = val; self.update_preview()
