...
class MockupBuddy(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MockupBuddy - PySide6 v0.6.7b")
    ...
    def init_ui(self):
        ...
        # Mockup Template Scroll Container (for future checkboxes)
        self.mockup_list_scroll = QScrollArea()
        self.mockup_list_scroll.setWidgetResizable(True)
        self.mockup_list_frame = QWidget()
        self.mockup_list_layout = QVBoxLayout(self.mockup_list_frame)
        self.mockup_list_scroll.setWidget(self.mockup_list_frame)

        # Optional: give the section a label
        control_layout.addWidget(QLabel("🧩 Mockup Templates"))
        control_layout.addWidget(self.mockup_list_scroll)
    ...
    def generate_mockups(self):
        ...
        for i, design_file in enumerate(designs):
            ...
            try:
                ...
                for mockup_file in os.listdir(self.mockup_folder):
                    if not mockup_file.lower().endswith((".png", ".jpg", ".jpeg")):
                        continue
                    mockup_path = os.path.join(self.mockup_folder, mockup_file)
                    mockup_img = Image.open(mockup_path).convert("RGBA")
                    is_dark = self.templates.get(mockup_file, {}).get("is_dark", False)
