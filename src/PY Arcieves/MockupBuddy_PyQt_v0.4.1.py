...
# MockupBuddy - PyQt Edition v0.4.1
# ✅ Fixed: Folder path persistence now correctly reloads previous session
# ✅ Fixed: Multiply blend mode no longer turns preview black
# ✅ Fixed: Preview loads on launch if persisted folders are found
# ✅ Confirmed: Collapse/Expand Advanced Options
# ⏳ Naming convention can be tuned later if needed
...
class MockupBuddy(QWidget):
    ...
    def load_images(self):
        # Load designs only if design_folder is set
        ...
        # Load mockups only if mockup_folder is set
    ...
    def overlay_design(self, mockup, design):
        ...
        new_size = (int(w * scale), int(h * scale)) if self.lock_aspect else (int(w * scale), int(h * scale))
        design = design.resize(new_size, Image.LANCZOS)
        if self.use_multiply:
            design = ImageChops.multiply(mockup.crop((0, 0, *design.size)), design)
        else:
            alpha = design.split()[3]
            alpha = alpha.point(lambda p: int(p * (self.opacity / 100)))
            design.putalpha(alpha)

        mockup.paste(design, ((mockup.width - design.width) // 2 + self.offset_x,
                              (mockup.height - design.height) // 2 + self.offset_y), design)
        return mockup
