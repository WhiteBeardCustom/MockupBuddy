...
class MockupBuddy(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MockupBuddy - PySide6 v0.6.7a")
    ...
    def init_ui(self):
        ...
        self.template_scroll.setMinimumHeight(150)
    ...
    def populate_template_checkboxes(self):
        ...
        for file in files:
            ...
            dark_check = QCheckBox("Dark BG")
            dark_check.setChecked(self.templates.get(file, {}).get("is_dark", False))
            dark_check.stateChanged.connect(
                lambda _, f=file, chk=dark_check: self.update_dark_flag(f, chk.isChecked())
            )
            hbox.addWidget(checkbox)
            hbox.addStretch()
            hbox.addWidget(dark_check)
            container = QWidget()
            container.setLayout(hbox)
            self.template_layout.addWidget(container)
            self.checkbox_vars.append((file, checkbox, dark_check))
