...
class MockupBuddy(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MockupBuddy - PySide6 v0.7.4")
    ...
    def reload_designs_and_mockups(self):
        ...
        # 🔁 Trigger preview if both selections are valid
    ...
    def generate_mockups(self):
        ...
        for i, design_file in enumerate(designs):
            base_name = get_design_basename(design_file)
            label.setText(f"Creating mockups for {base_name} ({i+1} of {len(designs)})")
            progress.setValue(i)
            popup.repaint()
            ...
            try:
                ...
                design_is_dark = is_dark_design(design_file)
                design_is_light = is_light_design(design_file)
                design_type = "neutral"
                if design_is_dark:
                    design_type = "dark"
                ...
                for mockup_file, is_dark in selected_mockups:
                    ...
                    # ✅ Fixed logic: Only apply matching designs to matching mockups
                    if (
                        (design_type == "dark" and not is_dark) or
                        (design_type == "light" and is_dark)
                    ):
                        self.log(f"⏭ Skipped: {design_file} → {mockup_file} (mismatch)")
                        continue
                    else:
                        self.log(f"✔ Matched: {design_file} → {mockup_file} (ok)")
    ...
    # [continue with overlay, output folder creation, saving, etc...]
                    ...
                    def extract_color_from_mockup(filename, known_prefixes=None):
                        name = os.path.splitext(filename)[0]
                        known_prefixes = known_prefixes or ["BellaCanvas3001", "BC3001", "NextLevel3600"]
                        for prefix in known_prefixes:
                            if name.startswith(prefix):
                                return name[len(prefix):].lstrip('_- ')
                        return name

                    sanitized_base = sanitize_name(design_file)
                    color_part = extract_color_from_mockup(mockup_file)
                    ...
                    mockup_count += 1
                    unique_basenames.add(get_design_basename(design_file))
